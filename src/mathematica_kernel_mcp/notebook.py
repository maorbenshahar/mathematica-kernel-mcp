...
_NOTEBOOK_WL_HELPERS = dedent(
    ...
    outputTagFor[inputCellId_String] := "WolframMCPOutputFor:" <> inputCellId;

    cellIdNumberOf[cell_Cell] := Module[{cellIds},
        cellIds = Cases[
            cell,
            HoldPattern[CellID -> value_Integer] :> value,
            Infinity
        ];
        If[cellIds === {}, Missing["NotAvailable"], First[cellIds]]
    ];

    hasOutputTag[cell_Cell, tag_String] := Module[{tags},
        tags = Cases[
            cell,
            HoldPattern[CellTags -> value_] :> value,
            Infinity
        ];
        MemberQ[Flatten[{tags}], tag]
    ];

    outputCellPosition[notebook_, inputCellId_String] := SelectFirst[
        Position[notebook, _Cell, Infinity, Heads -> False],
        hasOutputTag[Extract[notebook, #], outputTagFor[inputCellId]] &,
        Missing["NotFound"]
    ];

    buildOutputCell[result_, inputCellId_String, cellId_Integer] := Cell[
        BoxData[ToBoxes[result]],
        "Output",
        CellID -> cellId,
        CellTags -> {outputTagFor[inputCellId]}
    ];

    syncNotebookOutputs[rawNotebook_, specs_List] := Module[
        {
            baseNotebook,
            notebook,
            nextId,
        ...
        baseNotebook = normalizeNotebookIds[rawNotebook];
        notebook = baseNotebook;
        nextId = nextNotebookCellId[notebook];
        Do[
            inputCellId = Lookup[spec, "input_cell_id"];
            outNumber = Lookup[spec, "out_number"];
            If[!StringQ[inputCellId], Continue[]];
            inputIndex = resolveCellIndex[rawNotebook, inputCellId, Null];
            If[inputIndex === $Failed, inputIndex = resolveCellIndex[baseNotebook, inputCellId, Null]];
            If[inputIndex === $Failed || !IntegerQ[outNumber], Continue[]];
            canonicalInputCellId = cellIdOf[
                Extract[baseNotebook, cellPosition[baseNotebook, inputIndex]],
                inputIndex
            ];
            outputPos = outputCellPosition[notebook, canonicalInputCellId];
            If[
                outputPos === Missing["NotFound"],
                currentInputIndex = resolveCellIndex[notebook, canonicalInputCellId, Null];
                inputPos = cellPosition[notebook, currentInputIndex];
                outputId = nextId++;
                outputCell = buildOutputCell[
                    wolfram$mcp$out[outNumber],
                    canonicalInputCellId,
                    outputId
                ];
                notebook = insertCellAfterPosition[notebook, inputPos, outputCell],
                existingOutput = Extract[notebook, outputPos];
                outputId = Replace[cellIdNumberOf[existingOutput], _Missing :> nextId++];
                outputCell = buildOutputCell[
                    wolfram$mcp$out[outNumber],
                    canonicalInputCellId,
                    outputId
                ];
                notebook = ReplacePart[notebook, outputPos -> outputCell]
            ];
            AppendTo[
                synced,
                <|
                    "input_cell_id" -> canonicalInputCellId,
                    "output_cell_id" -> cellIdOf[outputCell, 0],
...
def sync_nb_output_cells(
    path: str | Path,
    manager: SessionManager,
    output_specs: list[dict[str, int | str]],
    *,
    session_name: str = "main",
) -> list[dict]:
    """Persist notebook output cells for previously evaluated input cells."""
    if not output_specs:
        return []
    ...
    payload = _load_json(
        ...
        _build_module(
            ...
            f"""
            Module[
                {{rawNotebook, specs, syncResult}},
                rawNotebook = Import[path, "NB"];
                specs = ImportString[{_wl_string(json.dumps(output_specs))}, "RawJSON"];
                syncResult = syncNotebookOutputs[rawNotebook, specs];
                Export[path, syncResult["notebook"], "NB"];
                ExportString[syncResult["synced"], "RawJSON"]
            ]
            """,
        ),
        session_name=session_name,
    ...
    if not isinstance(payload, list):
        raise ValueError("Unexpected notebook output sync payload")
    return payload
